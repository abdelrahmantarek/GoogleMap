...
class PolylineDecoded {
  ...
  //  --- Named Constructors
  /// .Decode(str, p)
  PolylineDecoded.Decode({this.encodedString, this.precision}) {
    decodedCoords = _decode(encodedString!, precision!);
    encodedString = encodedString;
  }

  /// .Encode(coords, p)
  PolylineDecoded.Encode({this.decodedCoords, this.precision}) {
    encodedString = _encode_poly(decodedCoords!, precision!);
    decodedCoords = decodedCoords;
  }

  // .Distance(str, u)
  PolylineDecoded.Distance({this.encodedString, this.unit}) {
  ...
  String _encode_poly(List<List<double>> coordinates, int precision) {
    ...
    num factor = pow(10, precision is int ? precision : 5);
    var output = _encode(coordinates[0][0], 0, factor as int) +
